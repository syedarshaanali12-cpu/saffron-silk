...
export default function Admin({c,s,setC,setS,loadError=''}) {
  ...
  if(!session) return <div className="admin-login"><form onSubmit={async event=>{event.preventDefault();setStatus('Signing in…');const {error}=await supabase.auth.signInWithPassword({email:ADMIN_EMAIL,password});setStatus(error?error.message:'Signed in')}}><p className="eyebrow">The Pilgrimage</p><h1>Admin login</h1><input type="email" value={ADMIN_EMAIL} readOnly aria-label="Admin email"/><input type="password" required minLength="6" placeholder="Password" value={password} onChange={event=>setPassword(event.target.value)} autoComplete="current-password"/><button className="button">Sign in</button><p>{status}</p></form></div>;
