...
export default function Admin({c,s,setC,setS,loadError=''}) {
  const [session,setSession]=useState(null),[password,setPassword]=useState(''),[newPassword,setNewPassword]=useState(''),[tab,setTab]=useState('home'),[status,setStatus]=useState(''),[saving,setSaving]=useState(false);
  ...
  const tabs=[['global','Brand & navigation'],['home',labels['/']||'Home'],['story',labels['/brand-story']||'Brand Story'],['events',labels['/events']||'Events'],['catering',labels['/catering']||'Catering'],['menu',labels['/menu']||'Menu'],['contact',labels['/contact']||'Contact'],['design','Design & effects']];
  const pageRoute=PAGE_ROUTES[tab];
  const renderEditor=()=>{
    if(tab==='global') return <><NavigationEditor navigation={c.shared.navigation} onChange={navigation=>setC({...c,shared:{...c.shared,navigation}})}/><section className="admin-card"><div className="admin-card-heading"><div><p className="eyebrow">Shared across the site</p><h2>Brand, footer and music</h2></div></div><ObjectEditor value={{shared:{...c.shared,navigation:undefined},opening:c.opening,music:c.music}} path="Global content" onUpload={uploadSiteMedia} onChange={next=>setC({...c,...next,shared:{...next.shared,navigation:c.shared.navigation}})}/></section></>;
