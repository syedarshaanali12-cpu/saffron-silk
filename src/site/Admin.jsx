...
function MenuEditor({menu, onChange, onUpload}) {
  ...
  return <section className="admin-card"><div className="admin-card-heading"><div><p className="eyebrow">Menu artwork</p><h2>Upload five PNGs</h2></div><p>Each file replaces one menu square. Nothing else needs editing.</p></div><div className="menu-admin-grid">{menu.sections.map((section,index) => <article className="menu-admin-card" key={index}><div className="menu-admin-number">Menu 0{index+1}</div>{section.image?<div className="menu-art-preview"><img src={section.image} alt={section.imageAlt||`Menu ${index+1}`}/></div>:<div className="menu-art-empty">No PNG uploaded yet</div>}<label className="admin-upload admin-upload--large"><b>{section.image?'Replace PNG':'Upload PNG'}</b><input type="file" accept="image/png" onChange={async event=>{const file=event.target.files?.[0];if(!file)return;const image=await onUpload(file);setSection(index,{...section,image,useImage:true,imageAlt:section.imageAlt||`Menu ${index+1}`})}}/></label></article>)}</div></section>;
}
